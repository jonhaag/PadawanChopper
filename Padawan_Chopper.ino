...
boolean isLeftStickDrive = false; 
...
int DomeServolow = 160;
int DomeServohigh =240;
...
#include <Wire.h>
#include <XBOXRECV.h>
//#include <Servo.h> //Servo conflicts with Softwareserial
//#include <PWMServo.h> //uaing PWM servo instead
...
//Servo headnodservo; //create servo object for the nod servo, the rest we control with the Maestro
//PWMServo headnodservo; //using pwnservo instead
...
void setup() {
  ...
  //headnodservo.attach(5);
//headnodservo.attach(SERVO_PIN_B); //uses pin 12 on the Arduino mega for the nod servo
...
if (Xbox.getButtonPress(L2, 0)) {
  ...
  if (Xbox.getButtonPress(L2, 0)) {
    if (Xbox.getButtonPress(LEFT, 0)) {
     maestrohead.restartScript(2);
    } 
  }

  if (Xbox.getButtonPress(L2, 0)) {
    if (Xbox.getButtonPress(RIGHT, 0)) {
     maestrohead.restartScript(3);
    } 
  }
...
//maestrohead.restartscript
  ...
  analogWrite(12,domeWobble); //write servo nod value to head nod servo

 // Serial.println (domeWobble);
